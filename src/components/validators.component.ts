import {Component, Binding, inject, CoreBindings} from '@loopback/core';
...
export class ValidatorsComponent implements Component {
  ...
  constructor(@inject(CoreBindings.APPLICATION_INSTANCE) private app: ApplicationWithServices) {
          ...
          validate: async ([model, field]: Array<any>, value: any) => {
            ...
            if (rows.length !== values.length) {
              ...
              const errors = valuesNotExists.map(
                v => ({message: `The value ${v} for model ${model} not exists`})
              )
